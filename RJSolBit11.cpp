...
static int b[512] = {        // Bitwise Candidates
  ...
  {62,71,80, 8,17,26,35,44,53,60,61,69,70,78,79}},
    ...
    1024,  2048,  4096,  8192, 16384, 32768, 65536,131072,262144,524288},
           W[99][8] = {      // XY-Wings (Type 1 and 2)and XYZ-Wings
  {19,25,46,52, 0, 0, 0, 0}, {10,12,64,66, 0, 0, 0, 0}, // 1 Hinge, 2 Wings,
  { 4, 6,76,78, 0, 0, 0, 0}, {18,25,27,34, 0, 0, 0, 0}, // Type 1: Wings common 1 Cell
  { 1,11,73,10,19,56,65,74}, {65,54,66,63,64,57,58,59}, // Type 2: Hinge and Wings common 2 Cells,
  {66, 3,77,57,75, 5,14,23}, {18,10,63, 0, 9,55,64,73}, //         Wings common 3 Cells
  {20, 1,23,18,19, 3, 4, 5}, {20,10,25,18,19,15,16,17},
  {70,43,78,61,79,33,42,51}, {11,18,65, 2,20,54,63,72},
  {33,43,69,42,51,61,70,79}, { 3,22,39,12,21,31,40,49},
  {22,14,26,21,23,15,16,17}, { 2, 5, 9, 0, 1,12,13,14},
  {27,30,37,28,29,39,40,41}, {53,35,45,51,52,27,28,29},
  {43,33,41,42,44,30,31,32}, {63, 0,65,54,72,02,11,20},
  {75,59,78,76,77,60,61,62}, {70,61,67,69,71,57,58,59},
  {48,21,40,30,39, 4,13,22}};
...
int solve (int p)
{
  ...
  for (a = 0; a < 4; ++a)    // Search XY-Wings Type 1
  {
    if (!g[W[a][0]] || !g[W[a][1]] || !g[W[a][2]] || !g[W[a][3]])
      continue;
...
#ifdef RJ
      printf ("%d) Found XY-Wing in Cells %d %d %d remove candidate %d from cell %d\n",
        a, W[a][0], W[a][1], W[a][2], b[g[W[a][1]] & g[W[a][2]]], W[a][3]);
#endif
      if (solve(p))
        return 1;
#ifdef RJ
      printf ("%d) Restore XY-Wing in Cells %d %d %d remove candidate %d from cell %d\n",
        a, W[a][0], W[a][1], W[a][2], b[g[W[a][1]] & g[W[a][2]]], W[a][3]);
#endif
      g[W[a][3]] = k;
      return 0;
...
#ifdef RJ
      printf ("%d) Found XY-Wing in Cells %d %d %d remove candidate %d from cell %d\n",
        a, W[a][1], W[a][0], W[a][3], b[g[W[a][0]] & g[W[a][3]]], W[a][2]);
#endif
      if (solve(p))
        return 1;
#ifdef RJ
...
#ifdef RJ
      printf ("%d) Found XY-Wing in Cells %d %d %d remove candidate %d from cell %d\n",
        a, W[a][2], W[a][0], W[a][3], b[g[W[a][0]] & g[W[a][3]]], W[a][1]);
#endif
      if (solve(p))
        return 1;
#ifdef RJ
...
#ifdef RJ
      printf ("%d) Found XY-Wing in Cells %d %d %d remove candidate %d from cell %d\n",
        a, W[a][3], W[a][1], W[a][2], b[g[W[a][1]] & g[W[a][2]]], W[a][0]);
#endif
      if (solve(p))
        return 1;
#ifdef RJ
  ...
  for (; a < 23; ++a)         // Search XY-Wings Type 2 and XYZ-Wings
  {
    if (!g[W[a][0]] || B[g[W[a][1]]] != 2 || B[g[W[a][2]]] != 2 ||
      ...
      (g[W[a][3]] | g[W[a][4]] | g[W[a][5]] | g[W[a][6]] | g[W[a][7]])))
    {
...
#ifdef RJ
      printf ("%d) Found XY-Wing in Cells %d %d %d remove candidate %d from cells %d %d %d %d %d\n",
      ...
      if (solve(p))
        return 1;
#ifdef RJ
      printf ("%d) Restore XY-Wing in Cells %d %d %d remove candidate %d from cells %d %d %d %d %d\n",
        a, W[a][0], W[a][1], W[a][2], b[g[W[a][1]] & g[W[a][2]]],
        W[a][3], W[a][4], W[a][5], W[a][6], W[a][7]);
#endif
      g[W[a][3]] = k[0];
    ...
    if (B[g[W[a][0]]] == 3 && B[g[W[a][1]] & g[W[a][2]]] == 1 &&
      (g[W[a][1]] & g[W[a][2]] & (g[W[a][3]] | g[W[a][4]])))
    {
      ...
      if (solve(p))
        return 1;
#ifdef RJ
      printf ("%d) Restore XYZ-Wing in Cells %d %d %d remove candidate %d from cells %d %d\n",
        a, W[a][0], W[a][1], W[a][2], b[g[W[a][1]] & g[W[a][2]]], W[a][3], W[a][4]);
#endif
      g[W[a][3]] = k[0];
      g[W[a][4]] = k[1];
