...
public:
   Zerodlx(int sz = 3, int maxSolns = 1) :
         BOX(sz),
         UNIT(BOX*BOX),
         GRID(UNIT*UNIT),
         maxSolutions(maxSolns),
         solution(GRID, 0),
         cols(GRID*4+1, column()),
         matrix(GRID*UNIT, std::vector<node>(4, node())) {
      head = &(cols[GRID*4]);
      int g4 = GRID*4+1;
      for (int i = 0; i < g4; i++) {  // construct the columns
      ...
      for (int i = 0; i < GRID*UNIT; i++)  // construct the rows
         for (int j = 0; j < 4; j++) {
            ...
            else if (j == 1)  // row-value
               colNum += i/GRID*UNIT+i%UNIT;
            else if (j == 2)  // col-value
               colNum += i%GRID/UNIT*UNIT+i%UNIT;
            else if (j == 3)  // box-value
   ...
   int solve(std::vector<int>& puzzle) {
      if (puzzle.size() != GRID)
         return -1;
      int k = 0;
      solnCount = 0;
      ...
      for (int i = 0; i < GRID; i++)
         if (puzzle[i]) {
            ...
            if (matrix[row][0].c->covered || matrix[row][1].c->covered ||
                matrix[row][2].c->covered || matrix[row][3].c->covered) {
               noSoln = true;
               break;
            }
            for (int j = 0; j < 4; j++)
      ...
      if (solnCount > 0)
         for (int i = 0; i < GRID; i++) {
            int pos = solution[i] / UNIT;
            puzzle[pos] = solution[i] % UNIT + 1;
         }
      for (int i = k - 1; i >= 0; i--)
         for (int j = 3; j >= 0; j--)
            uncover(matrix[solution[i]][j].c);
      return solnCount;
   }

private:
   // DLX recursive search(k) function - does the work
   void search(int k) {
      if (head->R == head) {
         solnCount++;
         return;
      ...
      cover(min);
      for (node *r = min->d; r != min && solnCount < maxSolutions; r = r->d) {
         if (solnCount == 0)
            solution[k] = r->row;
