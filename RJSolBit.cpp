...
int solve (int p)
{
  ...
  for (Y = 0; Y < 10; Y += 9)// Search Skyscraper and Grouped Skyscraper Line wise
    for (y = 1; y < 257; y <<= 1)
                             // Search Skyscraper and Grouped Skyscraper for each digit
      for (a = Y; a < Y + 6; ++a)
      {                      // Search first Strong Link within first and second chutes
        ...
        if (!X[0])
          continue;          // Skip for no first Strong Link Cell position found
        ...
        if (!X[1])
          continue;          // Skip for no first Strong Link Base and Cover Cell positions found
        for (int A = Y + (a < Y + 3 ? 3 : 6); A < Y + 9; ++A)
        {                    // Search second Strong Link within second and third chutes
          ...
          if (!X[3])
            continue;        // Skip for no second Strong Link Cell position found
          if ((X[6] = !(g[l[A][6]] & y) + !(g[l[A][7]] & y) + !(g[l[A][8]] & y)) == 2)
          {                  // Found second Strong Link either one Cover or Base Cell position
            if (X[4] || ~X[1] & 4)
          ...
          if (!X[4])
            continue;        // Skip for no second Strong Link Base and Cover Cell positions found
          X[6] = 0;
          ...
          int K[10] = {-1, 0,-1},
              k[4];
                             // Backup and exclude Skyscraper and Grouped Skyscraper Clue from excluding Cell positions
          X[6] = X[6] == X[0] ? 1 : 0;
          if (!X[2])
          {                  // Found first Strong Link one Cover Cell position
            K[0] = X[3 + X[6]];
...
printf ("%d) Found%sSkyscraper %ss %d %d Clue %d %d %d %d %d %d %d %d\n",
  p, X[5] ? " Grouped " : " ", Y ? "Column" : "Row", a - Y + 1, A - Y + 1,
  b[y], X[0], X[1], X[2], X[3], X[4], X[2] ? 0 : X[5], X[6]);
  ...
  for (a = 0; a < 81; ++a)   // Search Empty Rectangle Intersection Cell position grid wise
    for (int K[8] = {(g[a] | g[w[a][6]] | g[w[a][7]]) & (g[a] | g[w[a][12]] | g[w[a][13]]) &
      ~(g[w[a][8]] | g[w[a][9]] | g[w[a][10]] | g[w[a][11]])}; K[1] = K[0] & -K[0]; K[0] -= K[1])
                             // Assign ERI Cell Clues; and search Strong Link ERI digit wise
        ...
        {                    // Search second Wing Cell position Line wise
          ...
          for (Y = 0; Y < 2; ++Y)
            ...
            {                // Search third Wing Cell position
              ...
              if ((~g[K[0]] & K[4]) &&
                             // Check no Wing Cells common Clue Check Apex Cell Clues; and
