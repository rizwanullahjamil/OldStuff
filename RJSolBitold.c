...
static const int b[512] = {  // Bitwise to digit Cell Clues
  ...
  {62,71,80, 8,17,26,35,44,53,60,61,69,70,78,79}},
           W[15][2] = {      // References used for Basic, Hybrid and Transport Wings
...
int solve (int p)
{
    ...
    for (a = 0; a < 36; ++a) // Search Naked/Hidden pair Cell Clues for each Unit 36 pair Cell positions
    {
      ...
      if (B[K[0]] == 2)      // Check Naked pair Cell Clues in Unit pair Cell positions
      {
        ...
        printf ("%d) Found Naked pair %s %d Cells %d %d drop Clues %d from Cells %d %d %d %d %d %d %d\n",
          p, RCB, y % 9 + 1, l[y][h[a][0]], l[y][h[a][1]], b[K[0]], l[y][h[a][2]], l[y][h[a][3]],
      ...
      printf ("%d) Found Hidden pair %s %d Cells %d %d Clues %d\n",
        p, RCB, y % 9 + 1, l[y][h[a][0]], l[y][h[a][1]], b[K[0]]);
#endif
      if (solve (p))
        return 1;
#if RJ > 2
    ...
    for (; a < 120; ++a)     // Search Naked/Hidden triplet Cell Clues for each Unit 84 triplet Cell positions
    {
      ...
      if (B[K[0]] == 3)      // Check Naked triplet Cell Clues in Unit triplet Cell positions
      {
        ...
        printf ("%d) Found Naked triplet %s %d Cells %d %d %d drop Clues %d from Cells %d %d %d %d %d %d\n",
          p, RCB, y % 9 + 1, l[y][h[a][0]], l[y][h[a][1]], l[y][h[a][2]], b[K[0]], l[y][h[a][3]],
          l[y][h[a][4]], l[y][h[a][5]], l[y][h[a][6]], l[y][h[a][7]], l[y][h[a][8]]);
#endif
        if (solve (p))
          return 1;
      ...
      printf ("%d) Found Hidden triplet %s %d Cells %d %d %d Clues %d\n",
        p, RCB, y % 9 + 1, l[y][h[a][0]], l[y][h[a][1]], l[y][h[a][2]], b[K[0]]);
#endif
      if (solve (p))
        return 1;
#if RJ > 2
      printf ("%d) Restore Hidden triplet %s %d Cells %d %d %d Clues %d\n",
        p, RCB, y % 9 + 1, l[y][h[a][0]], l[y][h[a][1]], l[y][h[a][2]], y, b[K[0]]);
#endif
      g[l[y][h[a][0]]] = k[0];
      g[l[y][h[a][1]]] = k[1];
      g[l[y][h[a][2]]] = k[2];
    ...
    for (; a < 246; ++a)     // Search Naked/Hidden quad Cell Clues for each Unit 126 quad Cell positions
    {
      ...
      if (B[K[0]] == 4)      // Check Naked quad Cell Clues in Unit quad Cell positions
      {
        ...
        printf ("%d) Found Naked quad %s %d Cells %d %d %d %d drop Clues %d from Cells %d %d %d %d %d\n",
          p, RCB, y % 9 + 1, l[y][h[a][0]], l[y][h[a][1]], l[y][h[a][2]], l[y][h[a][3]], b[K[0]],
          l[y][h[a][4]], l[y][h[a][5]], l[y][h[a][6]], l[y][h[a][7]], l[y][h[a][8]]);
#endif
        if (solve (p))
          return 1;
  ...
  for (y = 0; y < 10; y += 9)// Search Skyscraper and Grouped Skyscraper Line wise
    for (Y = 1; Y < 257; Y <<= 1)
      ...
      {                      // Search 1st Strong Link within 1st and 2nd chutes
        int X[7] = {0},      // 1st and 2nd Strong Link Base and Cover Cell positions; and check for Grouped Cover Cell positions
            A;

        if ((X[6] = !(g[l[a][0]] & Y) + !(g[l[a][1]] & Y) + !(g[l[a][2]] & Y)) == 2)
          X[0] = 1;          // Found 1st Strong Link Base Cell position
        else if (X[6] < 2)
          X[0] = X[2] = 1;   // Found 1st Strong Link Grouped Cover Cell positions
        if ((X[6] = !(g[l[a][3]] & Y) + !(g[l[a][4]] & Y) + !(g[l[a][5]] & Y)) == 2)
          X[X[0]] = 2;       // Found 1st Strong Link either one Cover or Base Cell position
        else if (X[6] < 2)
        ...
        {                    // Search 2nd Strong Link within 2nd and 3rd chutes
          ...
          printf ("%d) Found%sSkyscraper %ss %d %d Base Cells %d %d Cover Cells",
            p, X[5] ? " Grouped " : " ", RCB, a - y + 1, A - y + 1, K[4], K[5]);
          if (K[6] + 1)
            printf (" %d", K[6]);
          if (K[7] + 1)
            printf (" %d", K[7]);
          ...
          {                  // No ERI digit in five Line Cell positions
            ...
            {                // Dual Linked Empty Rectangle exclusions
              ...
              printf ("%d) Found Dual Linked Empty Rectangle Intersection Cells %d %d Strong Link %s %d between Cell %d and Cell %d drop Clue %d from Cells %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
                p, a, K[7], R_C, b[(y ? w[K[2]][20] : w[K[2]][20] >> 18) & 511],
            ...
            printf ("%d) Found Empty Rectangle Intersection Cell %d Strong Link %s %d between Cell %d and Cell %d drop Clue %d from Cell %d\n",
              p, a, R_C, b[(y ? w[K[2]][20] : w[K[2]][20] >> 18) & 511],
  ...
  for (a = p; a < q; ++a)    // Search XY-Wings Type 1 and XY-Wings Type 1 Transport Apex unsolved Cell position wise
  {
    ...
    for (; K[1] < 6; ++K[1]) // Search 1st Wing Cell position Row wise
    {
      ...
      {                      // Search 2nd Wing Cell position Column wise
        ...
        {                    // Check Wing Cells common Clue in Apex and 1st Wing common Line but not Box (01, 05, 09)
          if ((g[w[K[0]][y = 12]] & (g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            ~(g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]) ||
            (g[w[K[0]][++y]] & ~(g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            (g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]))
          {                  // Check Wing Cells common Clue in one Line of Box within chute neither Apex and 1st Wing common Line nor Box (05, 09)
            k[0] = w[K[0]][y];
#if RJ > 2
            X[0][0] = Y;     // (05, 09)
            X[0][1] = w[Y][6];
        ...
        {                    // Check Wing Cells common Clue in Apex and 2nd Wing common Line but not Box (02, 06, 10)
          if ((g[w[K[0]][y = 6]] & (g[w[Y][6]] | g[w[Y][8]] | g[w[Y][10]]) &
            ~(g[w[Y][7]] | g[w[Y][9]] | g[w[Y][11]]) & K[5]) ||
            (g[w[K[0]][++y]] & ~(g[w[Y][6]] | g[w[Y][8]] | g[w[Y][10]]) &
            (g[w[Y][7]] | g[w[Y][9]] | g[w[Y][11]]) & K[5]))
          {                  // Check Wing Cells common Clue in one Line of Box within chute neither Apex and 1st Wing common Line nor Box (06, 10)
            k[0] = w[K[0]][y];
#if RJ > 2
            X[0][0] = Y;     // (06, 10)
            X[0][1] = w[Y][12];
        ...
        {                    // Check Wing Cells common Clue in 2nd Wing and exclusion common Line but not Box (03, 07, 11)
          if ((g[w[K[6]][y = 12]] & (g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            ~(g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]) ||
            (g[w[K[6]][++y]] & ~(g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            (g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]))
          {                  // Check Wing Cells common Clue in one Line of Box within chute neither Apex and 1st Wing common Line nor Box (07, 11)
            k[0] = w[K[6]][y];
#if RJ > 2
            X[0][0] = Y;     // (07, 11)
            X[0][1] = w[Y][6];
        ...
        {                    // Check Wing Cells common Clue in 1st Wing and exclusion common Line but not Box (04, 08, 12)
...
XYWT1Taf:
        ...
        if (k[1] + 1)
          g[k[1]] &= ~K[5];
#if RJ > 2
        printf ("%d) Found XY-Wings Type 1 Transport %s Cells %d %d %d Drop Cell %d Clues %d\nStrong Link Clue %d between Cells %d %d %d and Cell",
        ...
        if (solve (p))
          return 1;
#if RJ > 2
        printf ("%d) Restore XY-Wings Type 1 Transport %s Cells %d %d %d Drop Cell %d Clues %d\nStrong Link between Cells %d %d %d and Cell",
          p, (k[0] > -1) & (k[1] < 0) ? "Box" : (k[0] | k[1]) < 0 ? "ERI" : "Reduced ERI",
...
XYWT1Tb:
            ...
            if (~(g[Y = w[w[K[4]][12]][K[7]]] | g[w[Y][6]] | g[w[Y][7]]) & K[5])
            {
              ...
              {              // (Line Ring 19, 23)
                if ((~(g[w[K[6]][8]] | g[w[K[6]][9]]) & K[5]) &&
                  (((g[y = w[K[2]][K[8]]] | g[w[y][12]] | g[w[y][13]] |
                  g[w[K[2]][12]] | g[w[K[2]][13]] | g[w[K[4]][6]] | g[w[K[4]][7]] |
                  g[w[K[4]][10]] | g[w[K[4]][11]] | g[w[K[4]][13]] |
                  ((g[w[K[4]][8]] | g[w[K[4]][9]]) & K[5] ? 0 :
                  g[Y = w[K[0]][K[8]]] | g[w[Y][12]] | g[w[Y][13]] |
            ...
            if (~(g[Y = w[w[K[4]][13]][K[7]]] | g[w[Y][6]] | g[w[Y][7]]) & K[5])
            {
              ...
              {              // (Line Ring 19, 23)
                if ((~(g[w[K[6]][10]] | g[w[K[6]][11]]) & K[5]) &&
                  (((g[y = w[K[2]][K[8]]] | g[w[y][12]] | g[w[y][13]] |
                  g[w[K[2]][12]] | g[w[K[2]][13]] | g[w[K[4]][6]] | g[w[K[4]][7]] |
                  g[w[K[4]][8]] | g[w[K[4]][9]] | g[w[K[4]][12]] |
                  ((g[w[K[4]][10]] | g[w[K[4]][11]]) & K[5] ? 0 :
                  g[Y = w[K[0]][K[8]]] | g[w[Y][12]] | g[w[Y][13]] |
                  g[w[K[0]][12]] | g[w[K[0]][13]])) & K[5]) ||
                  ((g[w[K[0]][6]] | g[w[K[0]][7]] | g[w[K[2]][6]] | g[w[K[2]][7]] |
                  g[Y = w[K[0]][K[7]]] | g[w[Y][6]] | g[w[Y][7]]) & g[K[0]] & g[K[2]]) ||
            ...
            if (~(g[Y = w[w[K[2]][6]][K[8]]] | g[w[Y][12]] | g[w[Y][13]]) & K[5])
            {
              ...
              {              // (Line Ring 20, 24)
                if ((~(g[w[K[6]][8]] | g[w[K[6]][10]]) & K[5]) &&
                  (((g[y = w[K[4]][K[7]]] | g[w[y][6]] | g[w[y][7]] |
                  g[w[K[4]][6]] | g[w[K[4]][7]] | g[w[K[2]][7]] | g[w[K[2]][9]] |
                  g[w[K[2]][11]] | g[w[K[2]][12]] | g[w[K[2]][13]] |
            ...
            if (~(g[Y = w[w[K[2]][7]][K[8]]] | g[w[Y][12]] | g[w[Y][13]]) & K[5])
            {
              ...
              {              // (Line Ring 20, 24)
                if ((~(g[w[K[6]][9]] | g[w[K[6]][11]]) & K[5]) &&
                  (((g[y = w[K[4]][K[7]]] | g[w[y][6]] | g[w[y][7]] |
                  g[w[K[4]][6]] | g[w[K[4]][7]] | g[w[K[2]][6]] | g[w[K[2]][8]] |
                  g[w[K[2]][10]] | g[w[K[2]][12]] | g[w[K[2]][13]] |
       ...
       X[0][1] = -1;
#endif
        if ((g[y = w[K[2]][K[8]]] & ~(g[w[y][6]] | g[w[y][7]] | g[Y = w[K[0]][K[8]]]) & K[5]) ||
          (g[y += 9] & ~(g[w[y][6]] | g[w[y][7]] | g[Y += 9]) & K[5]) ||
          (g[y += 9] & ~(g[w[y][6]] | g[w[y][7]] | g[Y += 9]) & K[5]))
        {                    // (Line 37, 41)
#if RJ > 2
          X[0][0] = y;
#endif
          if (~(g[y = w[Y][K[7]]] | g[w[y][6]] | g[w[y][7]]) & K[5])
          {                  // (Line 37)
            if (g[w[Y][6]] & ~g[w[Y][7]] & (g[w[Y][12]] | g[w[Y][13]] |
              g[w[K[4]][6]] | g[w[K[4]][8]] | g[w[K[4]][10]]) & K[5])
            {                // (Line 37)
              k[3] = w[Y][12];
              k[47] = w[Y][13];
              k[0] = w[K[4]][6];
            ...
            else if (~g[w[Y][6]] & g[w[Y][7]] & (g[w[Y][12]] | g[w[Y][13]] |
              g[w[K[4]][7]] | g[w[K[4]][9]] | g[w[K[4]][11]]) & K[5])
            {                // (Line 37)
              k[3] = w[Y][12];
              k[47] = w[Y][13];
              k[0] = w[K[4]][7];
          ...
          else if ((~(g[w[Y][6]] | g[w[Y][7]]) & K[5]) &&
            ((g[y = w[Y][K[7]]] & g[K[4] + y - Y] & ~(g[w[y][6]] | g[w[y][7]]) & K[5]) ||
            (g[++y] & g[K[4] + y - Y] & ~(g[w[y][6]] | g[w[y][7]]) & K[5]) ||
            (g[++y] & g[K[4] + y - Y] & ~(g[w[y][6]] | g[w[y][7]]) & K[5])))
          {                  // (Line 41)
            k[3] = K[4] + y - Y;
#if RJ > 2
        ...
        if (g[y = w[K[4]][K[7]]] & ~(g[w[y][12]] | g[w[y][13]] | g[Y = w[K[0]][K[7]]]) & K[5] ||
          (g[++y] & ~(g[w[y][12]] | g[w[y][13]] | g[++Y]) & K[5]) ||
          (g[++y] & ~(g[w[y][12]] | g[w[y][13]] | g[++Y]) & K[5]))
        {                    // (Line 38, 42)
#if RJ > 2
          X[0][0] = y;
#endif
          if (~(g[y = w[Y][K[8]]] | g[w[y][12]] | g[w[y][13]]) & K[5])
          {                  // (Line 38)
            if (g[w[Y][12]] & ~g[w[Y][13]] & (g[w[Y][6]] | g[w[Y][7]] |
              g[w[K[2]][8]] | g[w[K[2]][9]] | g[w[K[2]][12]]) & K[5])
            {                // (Line 38)
              k[3] = w[Y][6];
              k[47] = w[Y][7];
              k[0] = w[K[2]][8];
            ...
            else if (~g[w[Y][12]] & g[w[Y][13]] & (g[w[Y][6]] | g[w[Y][7]] |
              g[w[K[2]][10]] | g[w[K[2]][11]] | g[w[K[2]][13]]) & K[5])
            {                // (Line 38)
              k[3] = w[Y][6];
              k[47] = w[Y][7];
              k[0] = w[K[2]][10];
          ...
          else if ((~(g[w[Y][12]] | g[w[Y][13]]) & K[5]) &&
            ((g[y = w[Y][K[8]]] & g[K[2] + y - Y] & ~(g[w[y][12]] | g[w[y][13]]) & K[5]) ||
            (g[y += 9] & g[K[2] + y - Y] & ~(g[w[y][12]] | g[w[y][13]]) & K[5]) ||
            (g[y += 9] & g[K[2] + y - Y] & ~(g[w[y][12]] | g[w[y][13]]) & K[5])))
          {                  // (Line 42)
            k[3] = K[2] + y - Y;
#if RJ > 2
        ...
        if (g[y = w[K[0]][K[8]]] & ~(g[w[y][6]] | g[w[y][7]] | g[Y = w[K[2]][K[8]]]) & K[5] ||
          (g[y += 9] & ~(g[w[y][6]] | g[w[y][7]] | g[Y += 9]) & K[5]) ||
          (g[y += 9] & ~(g[w[y][6]] | g[w[y][7]] | g[Y += 9]) & K[5]))
        {                    // (Line 39, 43)
#if RJ > 2
          X[0][0] = y;
#endif
          if (~(g[L = w[y][K[7]]] | g[w[L][6]] | g[w[L][7]]) & K[5])
          {                  // (Line 39)
            if (g[w[Y][6]] & ~g[w[Y][7]] & (g[w[Y][12]] | g[w[Y][13]] |
              g[w[K[2]][6]] | g[w[K[2]][8]] | g[w[K[2]][10]]) & K[5])
            {                // (Line 39)
              k[3] = w[Y][12];
              k[47] = w[Y][13];
              k[0] = w[K[2]][6];
              ...
              g[w[K[2]][7]] | g[w[K[2]][9]] | g[w[K[2]][11]]) & K[5])
            {                // (Line 39)
              k[3] = w[Y][12];
              k[47] = w[Y][13];
              k[0] = w[K[2]][7];
            ...
            ((g[L = w[y][K[7]]] & g[K[2] + L - Y] & ~(g[w[L][6]] | g[w[L][7]]) & K[5]) ||
            (g[++L] & g[K[2] + L - Y] & ~(g[w[L][6]] | g[w[L][7]]) & K[5]) ||
            (g[++L] & g[K[2] + L - Y] & ~(g[w[L][6]] | g[w[L][7]]) & K[5])))
          {                  // (Line 43)
            k[3] = K[2] + L - Y;
#if RJ > 2
        ...
        if (g[y = w[K[0]][K[7]]] & ~(g[w[y][12]] | g[w[y][13]] | g[Y = w[K[4]][K[7]]]) & K[5] ||
          (g[++y] & ~(g[w[y][12]] | g[w[y][13]] | g[++Y]) & K[5]) ||
          (g[++y] & ~(g[w[y][12]] | g[w[y][13]] | g[++Y]) & K[5]))
        {                    // (Line 40, 44)
#if RJ > 2
          X[0][0] = y;
#endif
          if (~(g[L = w[y][K[8]]] | g[w[L][12]] | g[w[L][13]]) & K[5])
          {                  // (Line 40)
            if (g[w[Y][12]] & ~g[w[Y][13]] & (g[w[Y][6]] | g[w[Y][7]] |
              g[w[K[4]][8]] | g[w[K[4]][9]] | g[w[K[4]][12]]) & K[5])
            {                // (Line 40)
              k[3] = w[Y][6];
              k[47] = w[Y][7];
              k[0] = w[K[4]][8];
              ...
              g[w[K[4]][10]] | g[w[K[4]][11]] | g[w[K[4]][13]]) & K[5])
            {                // (Line 40)
              k[3] = w[Y][6];
              k[47] = w[Y][7];
              k[0] = w[K[4]][10];
            ...
            ((g[L = w[y][K[8]]] & g[K[4] + L - Y] & ~(g[w[L][12]] | g[w[L][13]]) & K[5]) ||
            (g[L += 9] & g[K[4] + L - Y] & ~(g[w[L][12]] | g[w[L][13]]) & K[5]) ||
            (g[L += 9] & g[K[4] + L - Y] & ~(g[w[L][12]] | g[w[L][13]]) & K[5])))
          {                  // (Line 44)
            k[3] = K[4] + L - Y;
#if RJ > 2
...
XYWT1Tbf:
  ...
  for (a = p; a < q; ++a)    // Search XY-Wings Type 2, XY-Wings Type 2 Transport, XYZ-Wings and XYZ-Wings Hybrid Type 1 and Type 2 Apex unsolved Cell position wise
  {
    if (B[g[r[a]]] > 3)
      ...
      {                      // Search 1st Wing Cell position Line wise
        ...
        {                    // Search 2nd Wing Cell position Box wise but not 1st Wing Line
          ...
          int k[5] = {g[w[K[0]][W[3][y]]], g[w[K[0]][W[4][y]]]};
                             // Backup XY-Wings Type 2 and XYZ-Wings more exclusion Cells Clues
          if (~g[K[0]] & K[5])
          {                  // Check no Wing Cells common Clue in Apex Cell Clues for backup more XY-Wings Type 2 exclusion Cells Clues
            k[2] = g[w[K[4]][K[6]]];
            k[3] = g[w[K[4]][K[6] + 1]];
            k[4] = g[w[K[4]][K[6] + 2]];
          }
          ...
          {                  // Check Wing Cells common Clue in XY-Wings Type 2 and XYZ-Wings exclusion Cells Clues
            ...
            g[w[K[0]][W[4][y]]] &= ~K[5];
            if (~g[K[0]] & K[5])
            {                // Drop Wing Cells common Clue from XY-Wings Type 2 more exclusion Cell positions
              g[w[K[4]][K[6]]] &= ~K[5];
              g[w[K[4]][K[6] + 1]] &= ~K[5];
              g[w[K[4]][K[6] + 2]] &= ~K[5];
            }
            ...
            printf ("%d) Found XY%s Cells %d %d %d Clues %d drop Clue %d from Cells %d %d",
              p, g[K[0]] & K[5] ? "Z-Wings" : "-Wings Type 2", K[0], K[2], K[4],
            ...
            g[w[K[0]][W[4][y]]] = k[1];
            if (~g[K[0]] & K[5])
            {                // Restore Wing Cells common Clue to XY-Wings Type 2 more exclusion Cell positions
              g[w[K[4]][K[6]]] = k[2];
              g[w[K[4]][K[6] + 1]] = k[3];
              g[w[K[4]][K[6] + 2]] = k[4];
            }
          ...
          {                  // Wing Cells common Clue not in Apex Cell Clues
            ...
            for (A = 0; A < 3; ++A)
            {                // (Line 09, 10, ERI 13, 14)
              if (g[L = w[K[0]][K[7] + A]] & K[5])
                for (Y = W[1][!y]; Y < W[2][!y]; ++Y)
                  if (k[75] = (g[X = w[L][Y]] & ~(g[w[X][W[3][y]]] | g[w[X][W[4][y]]] |
                    g[M = w[K[2]][Y]] | g[w[M][W[3][y]]] | g[w[M][W[4][y]]] |
                    g[w[Z = w[K[4]][Y]][W[3][y]]] | g[w[Z][W[4][y]]]) & g[Z] & K[5]) ||
                    (ERI (X) & ERI (Z) & K[5]))
                  {          // (Line 09, ERI 13)
                    k[0] = L;
#if RJ > 2
              ...
              if (~g[L = w[K[4]][K[7] + A]] & K[5])
                continue;
              for (Y = W[1][!y]; Y < W[2][!y]; ++Y)
                if (k[75] = (g[X = w[L][Y]] & ~(g[w[X][W[3][y]]] | g[w[X][W[4][y]]] |
                  g[M = w[K[4]][Y]] | g[w[M][W[3][y]]] | g[w[M][W[4][y]]] |
                  g[w[Z = w[K[2]][Y]][W[3][y]]] | g[w[Z][W[4][y]]]) & g[Z] & K[5]) ||
                  (ERI (X) & ERI (Z) & K[5]))
                {            // (Line 10, ERI 14)
                  k[0] = L;
#if RJ > 2
            ...
            M = w[K[4]][W[K[0] == w[K[4]][W[6][y]] || K[0] == w[K[4]][W[7][y]] ||
                K[0] == w[K[4]][W[8][y]] ? 4 : 3][!y]];
            for (Y = W[1][!y]; Y < W[2][!y]; ++Y)
            {                // (Line 11, 12, ERI 15, 16, Line Ring 22, ERI Ring 23)
              for (A = 3; A < 5; ++A)
              {              // (Line 11, 12, ERI 15, 16)
                if (((g[L = w[K[2]][W[A][y]]] | g[w[L][K[8]]] |
                  g[w[Z = w[K[2]][Y]][W[3][!y]]] | g[w[Z][W[4][!y]]]) & K[5]) &&
                  (k[75] = (g[X = w[L][Y]] & ~(g[w[X][W[3][y]]] | g[w[X][W[4][y]]] |
                  g[Z = w[w[K[4]][Y]][K[7]]] | g[w[Z][W[3][y]]] | g[w[Z][W[4][y]]] |
                  g[w[Z = w[K[4]][Y]][W[3][y]]] | g[w[Z][W[4][y]]]) & g[Z] & K[5]) ||
                  (ERI (X) & ERI (Z) & K[5])))
                {            // (Line 11, ERI 15)
                  k[0] = L;
                ...
                if (((g[L = w[K[4]][W[A][y]]] | g[w[L][K[8]]] |
                  g[w[Z = w[K[4]][Y]][W[3][!y]]] | g[w[Z][W[4][!y]]]) & K[5]) &&
                  (k[75] = (g[X = w[L][Y]] & ~(g[w[X][W[3][y]]] | g[w[X][W[4][y]]] |
                  g[Z = w[w[K[0]][Y]][K[7]]] | g[w[Z][W[3][y]]] | g[w[Z][W[4][y]]] |
                  g[w[Z = w[K[2]][Y]][W[3][y]]] | g[w[Z][W[4][y]]]) & g[Z] & K[5]) ||
                  (ERI (X) & ERI (Z) & K[5])))
                {            // (Line 12, ERI 16)
                  k[0] = L;
            ...
            X = M;
            Z = w[K[2]][K[8]];
            if ((~(g[L = w[M][K[7]]] | g[w[L][W[3][y]]] | g[w[L][W[4][y]]]) & K[5]) &&
              ((k[75] = (g[X = M] | g[w[X][W[3][y]]] | g[w[X][W[4][y]]]) &
              (g[Z = w[K[2]][K[8]]] | g[w[Z][W[3][y]]] | g[w[Z][W[4][y]]]) & K[5]) ||
              ((g[X = w[K[0]][K[7]]] | g[w[X][W[3][y]]] | g[w[X][W[4][y]]]) &
              (g[Z = w[K[4]][K[7]]] | g[w[Z][W[3][y]]] | g[w[Z][W[4][y]]]) & K[5])) &&
              (((g[w[K[2]][W[3][y]]] | g[w[K[2]][W[4][y]]] | g[w[K[4]][W[3][y]]] |
              g[w[K[4]][W[4][y]]] | (~(g[w[X][W[3][y]]] | g[w[X][W[4][y]]]) & K[5] ?
              ...
              g[M] | g[w[M][W[3][y]]] | g[w[M][W[4][y]]] |
              g[w[K[4]][W[3][y]]] | g[w[K[4]][W[4][y]]]) & g[K[0]] & g[K[4]])))
            {                // (Line Reduced Ring 17, Box Reduced Ring 18, Line Ring 19, 20, 21)
              k[0] = w[K[2]][W[3][y]];
              k[1] = w[K[2]][W[4][y]];
              k[2] = w[K[4]][W[3][y]];
              k[5] = 2;
...
XYWT2Tf:
...
#if RJ > 2
            printf ("%d) Found XY-Wings Type 2 Transport %s%sCells %d %d %d Clues %d\nStrong Link %s Cells %d %d drop Clue %d from",
              p, k[75] > 2 ? "ERI" : R_C, k[5] ? ((k[3] & k[4]) < 0 ? " Reduced Ring " : " Ring ") : " ",
              K[0], K[2], K[4], b[g[K[2]] | g[K[4]]], k[75] > 2 ? "ERI" : R_C, X, Z, b[K[5]]);
            if (k[0] + 1)
            {
              printf (" %d", k[0]);
              if (k[1] + 1)
            ...
            if (k[3] + 1)
              printf (" %d %d %d %d %d %d", k[3], k[7], k[8], k[9], k[10], k[11]);
            if (k[4] + 1)
              printf (" %d %d %d %d %d %d", k[4], k[12], k[13], k[14], k[15], k[16]);
            printf ("\n");
            if (k[5])
              printf ("drop Clue %d from %d %d %d %d %d %d %d drop Clue %d from %d %d %d %d %d %d %d\n",
                b[g[K[0]] & g[K[2]]], k[47], k[48], k[49], k[50], k[51], k[52], k[53],
                b[g[K[0]] & g[K[4]]], k[54], k[55], k[56], k[57], k[58], k[59], k[60]);
#endif
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Restore XY-Wings Type 2 Transport %s%sCells %d %d %d Clues %d\nStrong Link %s Cells %d %d add Clue %d to",
              p, k[75] > 2 ? "ERI" : R_C, k[5] ? (k[3] + 1 ? " Ring " : "Reduced Ring ") : " ",
              K[0], K[2], K[4], b[g[K[2]] | g[K[4]]], k[75] > 2 ? "ERI" : R_C, X, Z, b[K[5]]);
            if (k[0] + 1)
            {
              printf (" %d", k[0]);
          ...
          {                  // Search XYZ-Wings Hybrid Type wise
            ...
            {                // Search XYZ-Wings Hybrid Type 1
              X[0] = W[K[1] < W[5][y] ? 5 : 1][y];
              X[1] = X[0] + 1;
              X[2] = X[1] + 1;
            }
            else
            {                // Search XYZ-Wings Hybrid Type 2
              L = K[3] == W[6][y] || K[3] == W[7][y] || K[3] == W[8][y] ? 9 : 6;
              X[0] = W[L][y];
              X[1] = W[L + 1][y];
              X[2] = W[L + 2][y];
            }
            if ((g[K[4 >> Y]] != g[w[K[4]][L = K[6]]] &&
              g[K[4 >> Y]] != g[w[K[4]][++L]] && g[K[4 >> Y]] != g[w[K[4]][++L]]) ||
              (~(g[w[K[0]][X[0]]] | g[w[K[0]][X[1]]] | g[w[K[0]][X[2]]]) & K[5]))
            ...
            printf ("%d) Found XYZ-Wings Hybrid Type %d Cells %d %d %d %d Clues %d drop Clue %d from Cells %d %d %d\n",
              p, 2 - Y, K[0], K[2], K[4], w[K[4]][L], b[g[K[0]]],
              b[K[5]], w[K[0]][X[0]], w[K[0]][X[1]], w[K[0]][X[2]]);
#endif
            if (solve (p))
              return 1;
#if RJ > 2
  ...
  for (y = p; y < q - 1; ++y)// Search W-Wings Type 1 and Type 2 1st Cell position unsolved Cell position wise
  {
    if (B[g[r[y]]] > 2)
      continue;              // Skip for 1st Cell Clues > two digits
    for (Y = y + 1; Y < q; ++Y)
    {                        // Search W-Wings Type 1 and Type 2 2nd Cell position unsolved Cell position wise
      if ((w[r[y]][20] & w[r[Y]][20] & 134217727) || g[r[y]] != g[r[Y]])
        continue;            // Skip for either 1st and 2nd Cell positions in Unit; or Clues not same
      ...
      {                      // Check 1st and 2nd Cell positions either Band or Stack wise
        ...
        {                    // Search W-Wings Type 1 Strong Link Line and ERI wise
          ...
          if ((Z = ERI (L = K[3 + a]) & K[2]) && ((k[0] | k[1]) & (K[2] - Z)))
      ...
      for (a = X[2][0]; a <= X[2][1]; ++a)
        ...
        {                    // Search W-Wings Type 1 and Type 2 Strong Link Line wise
          ...
          if ((Z = ERI (w[K[0]][L]) & ERI (w[K[3 + a]][L]) & K[2]) &&
            ((k[0] | k[1] | k[2] | k[3] | k[4] | k[5]) & (K[2] - Z)))
          {                  // Check Strong Link ERI wise
            K[2] -= Z;
            g[K[3]] &= ~K[2];
  ...
  for (a = p; a < q; ++a)    // Search WXYZ-Wings Type 1 Apex unsolved Cell position wise
  {
    ...
    for (; K[1] < 8; ++K[1]) // Search 1st Wing Cell position Row wise
    {
      ...
      {                      // Search 2nd Wing Cell position Column wise but not both 1st and 2nd Wings in Apex Box
        if (!g[K[4] = w[K[0]][K[3]]] || B[g[K[0]] | g[K[2]] | g[K[4]]] > 4)
          continue;          // Skip for either 2nd Wing Cell position not unsolved; or Apex, 1st and 2nd Wings Cells Clues > four digits
        for (y = 0; y < 2; ++y)
        {                    // Search 3rd Wing Cell position Line wise
          ...
          {                  // Search 3rd Wing Cell position
            ...
            {                // Check no Wing Cells common Clue in WXYZ-Wings Type 1 exclusion Cell Clues
              if (k[5] + 1)
              {              // Drop Wing Cells common Clue from WXYZ-Wings Type 1 exclusion Cell positions
                g[w[K[0]][W[3][k[5]]]] &= ~K[7];
                g[w[K[0]][W[4][k[5]]]] &= ~K[7];
                if (~g[K[0]] & K[7])
                {
                ...
                g[K[8]] &= ~K[7];
#if RJ > 2
              printf ("%d) Found WXYZ-Wings Type 1 Cells %d %d %d %d Clues %d drop Clue %d from Cell",
              ...
              if (solve (p))
                return 1;
#if RJ > 2
              printf ("%d) Restore WXYZ-Wings Type 1 Cells %d %d %d %d Clues %d add Clue %d to Cell",
                p, K[0], K[2], K[4], K[6], b[g[K[2]] | g[K[4]] | g[K[6]]], b[K[7]]);
              if (k[5] + 1)
              {
                printf ("s %d %d", w[K[0]][W[3][k[5]]], w[K[0]][W[4][k[5]]]);
                ...
                printf (" %d\n", K[8]);
#endif
              if (k[5] + 1)  // Restore Wing Cells common Clue to WXYZ-Wings Type 1 exclusion Cell positions
              {
                g[w[K[0]][W[3][k[5]]]] = k[3];
                g[w[K[0]][W[4][k[5]]]] = k[4];
                if (~g[K[0]] & K[7])
...
// WXYZ-Wings Type 1 Transport
  ...
  for (a = p; a < q; ++a)    // Search WXYZ-Wings Type 2a, Type 2b, Type 3, Type 4a and Type 4b Apex unsolved Cell position wise
  {
          ...
          for (Y = 0; Y < 2; ++Y)
            ...
            {                // Search 3rd Wing Cell position
              ...
              {              // Wing Cells common Clue not in both 1st (or 2nd) Wing Cell Clues and 3rd Wing Cell Clues
                X[0] = K[g[K[2]] & g[K[4 >> Y]] & (g[K[2 << Y]] | g[K[6]]) ? 2 : 6];
                X[1] = W[K[g[K[4]] & g[K[4 >> Y]] & (g[K[2 << Y]] | g[K[6]]) ? 3 : 5] < W[5][y] ? 1 : 5][y];
                k[2] = g[w[X[0]][X[1]]];
                k[3] = g[w[X[0]][X[1] + 1]];
                k[4] = g[w[X[0]][X[1] + 2]];
              ...
              {              // Check WXYZ-Wings Type 2a, Type 2b and Type 3 exclusion Cells Clues
                ...
                if (X[0] + 1)
                  printf ("%d) Found WXYZ-Wings Type 2a Cells %d %d %d %d Clues %d drop Clue %d from Cells %d %d %d %d %d\n",
                    ...
                    b[K[7]], w[K[0]][W[3][y]], w[K[0]][W[4][y]], w[X[0]][X[1]], w[X[0]][X[1] + 1], w[X[0]][X[1] + 2]);
                else
          ...
          {                  // Search 2nd Apex Cell position mini-Line wise
            ...
            {                // Check WXYZ-Wings Type 4a and Type 4b exclusion Cells Clues
              ...
              printf ("%d) Found WXYZ-Wings Type 4%c Cells %d %d %d %d Clues %d drop Clue %d from Cell",
                p, L + 1 ? 'a' : 'b', K[0], K[6], K[2], K[4], b[g[K[0]] | g[K[2]] | g[K[4]] | g[K[6]]], b[K[7]]);
              if (L + 1)
                printf ("s %d %d %d %d\n", w[K[0]][W[7 - K[5]][y]], w[K[2]][L], w[K[2]][L + 1], w[K[2]][L + 2]);
              else
                printf (" %d\n", w[K[0]][W[7 - K[5]][y]]);
#endif
              if (solve (p))
