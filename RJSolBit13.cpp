...
#ifndef RJ
#define RJ 1                 // 0 no debugging, 1 print solutions, 2 print steps, 3 print pencil marks
#endif
  ...
  for (; y < 27; ++y)        // Search Naked/Hidden Tuples for each Unit
  {
    ...
    for (; a < 120; ++a)     // Search Naked/Hidden triplet Candidates for each Unit 84 triplet Cell positions
    {
      ...
      int K[2] = {g[l[y][h[a][0]]] | g[l[y][h[a][1]]] | g[l[y][h[a][2]]],
        g[l[y][h[a][3]]] | g[l[y][h[a][4]]] | g[l[y][h[a][5]]] |
        g[l[y][h[a][6]]] | g[l[y][h[a][7]]] | g[l[y][h[a][8]]]};
                             // Assign Candidates in Unit triplet and other Cell positions
    ...
    for (; a < 246; ++a)     // Search Naked/Hidden quad Candidates for each Unit 126 quad Cell positions
    {
      ...
      int K[2] = {g[l[y][h[a][0]]] | g[l[y][h[a][1]]] | g[l[y][h[a][2]]] |
        g[l[y][h[a][3]]], g[l[y][h[a][4]]] | g[l[y][h[a][5]]] |
        g[l[y][h[a][6]]] | g[l[y][h[a][7]]] | g[l[y][h[a][8]]]};
                             // Assign Candidates in Unit quad and other Cell positions
        ...
        {                    // Search bottom left Cell position
          ...
          {                  // Search four combinations of Pivot, Pincers and elimination Cell positions
            if (B[g[K[W[a][0]]] | g[K[W[a][1]]] | g[K[W[a][2]]]] != 3 ||
              ...
              continue;      // Found Cells contain either not three candidates or common candidate or no elimination
            int k = g[K[W[a][3]]];
                             // Backup XY-Wing Type 1 elimination Cell candidates and eliminate XY-Wing Type 1 candidate from elimination Cell position
...
#endif
            g[K[W[a][3]]] = k;
            return 0;        // Restore XY-Wing Type 1 elimination Cell candidate
          }
        }
      }
    }
  for (a = 0; a < 2; ++a)    // Search XY-Wings Type 2 and XYZ-Wings Box-Line wise
    for (y = 0; y < 81; ++y) // Search Pivot Cell wise
    {
      if (B[g[y]] < 2 || B[g[y]] > 3)
        continue;            // Found Pivot Cell contain either less than two candidates or greater than three candidates
      for (Y = W[4][a]; Y < W[5][a]; ++Y)
      {                      // Search first Pincer Cell Box wise
        if (B[g[w[y][Y]]] != 2 || B[g[y] | g[w[y][Y]]] != 3)
          continue;          // Found either Box Pincer not contain two candidates or Pivot and Box Pincer not contain not three candidates
        for (int K = W[6][a]; K < W[7][a]; ++K)
        {                    // Search second Pincer Cell Line wise
          if (B[g[w[y][K]]] != 2 || B[g[y] | g[w[y][Y]] | g[w[y][K]]] != 3)
            continue;        // Found either Line Pincer not contain two candidates or Cells not contain three candidates or contain common candidate
          if (!(g[y] & g[w[y][Y]] & g[w[y][K]]))
          {                  //  Not found Cells contain common candidate for XY-Wing Type 2
            int L = W[K < W[10][a] ? 6 : 10][a],
                k[5] = {g[w[y][W[8][a]]], g[w[y][W[9][a]]],
                  g[w[w[y][Y]][L]], g[w[w[y][Y]][L + 1]], g[w[w[y][Y]][L + 2]]};
                             // Backup XY-Wing Type 2 elimination Cells candidates
            if (!(g[w[y][Y]] & g[w[y][K]] & (k[0] | k[1] | k[2] | k[3] | k[4])))
              continue;      // Not found Pincers common candidate in elimination Cell positions
            g[w[y][W[8][a]]] &= ~(g[w[y][Y]] & g[w[y][K]]);
            g[w[y][W[9][a]]] &= ~(g[w[y][Y]] & g[w[y][K]]);
            g[w[w[y][Y]][L]] &= ~(g[w[y][Y]] & g[w[y][K]]);
            g[w[w[y][Y]][L + 1]] &= ~(g[w[y][Y]] & g[w[y][K]]);
            g[w[w[y][Y]][L + 2]] &= ~(g[w[y][Y]] & g[w[y][K]]);
#if RJ > 1
            printf ("%d) Found XY-Wing Type 2 in Cells %d %d %d remove candidate %d from cells %d %d %d %d %d\n",
              a, y, w[y][Y], w[y][K], b[g[w[y][Y]] & g[w[y][K]]], w[y][W[8][a]], w[y][W[9][a]],
              w[w[y][Y]][L], w[w[y][Y]][L + 1], w[w[y][Y]][L + 2]);
#endif
            if (solve (p))
              return 1;
#if RJ > 1
          ...
          if (B[g[w[y][Y]] & g[w[y][K]]] != 1 ||
            B[g[y] & g[w[y][Y]] & g[w[y][K]]] != 1 ||
            !(g[w[y][Y]] & g[w[y][K]] & (g[w[y][W[8][a]]] | g[w[y][W[9][a]]])))
            continue;        // Not found either Pincers one common candidate or Cells one common candidate or eliminations
          int k[2] = {g[w[y][W[8][a]]], g[w[y][W[9][a]]]};
                             // Backup XYZ-Wing elimination Cells candidates and eliminate common Pincers candidate from elimination Cell positions
          g[w[y][W[8][a]]] &= ~(g[w[y][Y]] & g[w[y][K]]);
          g[w[y][W[9][a]]] &= ~(g[w[y][Y]] & g[w[y][K]]);
#if RJ > 1
