...
int solve (int p)
{
  ...
  for (a = p; a < q; ++a)    // Search XY-Wings Type 1 unsolved Cell position wise
  {
    ...
    {                        // Assign Pivot Cell position and search first Pincer Cell position Row wise
      ...
      {                      // Search second Pincer Cell position Column wise
        ...
        if (B[g[w[K[0]][K[2]]]] != 2 ||
          ...
          B[K[3]] != 1 ||    // Pincer Cells common Value not one digit; or
          g[K[0]] & K[3] ||  // Pivot and Pincer Cells common Value; or
          B[g[K[0]] | g[w[K[0]][K[1]]] | g[w[K[0]][K[2]]]] != 3)
          continue;          // Combined Pivot and Pincer Cells Values not three digits
        if (K[3] & g[K[4]])  // Search XY-Wings Type 1 elimination Cell Values
        {
...
// XY-Wings Transport Type 1
  ...
  for (y = 0; y < 2; ++y)    // Search XY-Wings Type 2, XYZ-Wings and XYZ-Wings Hybrid Type 1 and Type 2 Chute wise
    for (a = p; a < q; ++a)  // Search unsolved Cell position wise
    {
      ...
      for (int K[5] = {r[a], W[0][y]}; K[1] < W[1][y]; ++K[1])
      {                      // Assign Pivot Cell position and search first Pincer Cell position Box wise
