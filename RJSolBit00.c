...
// XYZ-Wings Transport
  ...
  for (y = p; y < q - 1; ++y)// Search W-Wings Type 1, Type 2 and W-Rings first Cell position unsolved Cell position wise
  {
    if (B[g[r[y]]] != 2)
      continue;              // Skip for first Cell Clues not two digits
    for (Y = y + 1; Y < q; ++Y)
    {                        // Search W-Wings Type 1, Type 2 and W-Rings second Cell position unsolved Cell position wise
      ...
      {                      // Check first and second Cell positions eitehr Band or Stack wise
        ...
        for (a = 0; K[2] && a < 2; ++a)
        {                    // Search W-Wings Type 1 and W-Rings Strong Link Line and ERI wise
          for (K[6] = 0; K[2] && K[6] < 2; ++K[6])
            for (K[7] = 0; K[2] && K[7] < 2; ++K[7])
      ...
      for (a = X[2][0]; K[2] && a <= X[2][1]; ++a)
        for (L = X[0][a]; K[2] && L < X[1][a]; ++L)
        {                    // Search W-Wings Type 1, Type 2 and W-Rings Strong Link Line wise
      ...
      else if (K[9])
      {
...
#if RJ > 2
        printf ("%d) Found W-Wings Type %s Cells %d %d Clues %d Strong Link ERI Cells %d %d Clue %d drop Clue %d from %d ",
        ...
        if (solve (p))
          return 1;
#if RJ > 2
        printf ("%d) Restore W-Wings Type %s Cells %d %d Clues %d Strong Link ERI Cells %d %d Clue %d add Clue %d to %d ",
          p, X[2][0] != X[2][1] ? "1 Grid" : X[2][0] ? "2 Stack" : "2 Band",
          K[0], K[1], b[g[K[0]]], w[K[0]][L], w[K[3 + a]][L], b[Z], b[g[K[0]] - Z], K[3]);
        if (K[5] < 0)
          printf ("%d\n", K[4]);
...
// WXYZ-Wings Type 2a, Type 2b and Type 3 Transport
  ...
  {
    int K[5] = {(g[a] | g[w[a][6]] | g[w[a][7]]) & (g[a] | g[w[a][12]] | g[w[a][13]]) &
    ...
    for (; K[1] = K[0] & -K[0]; K[0] -= K[1])
      ...
      for (y = 0; y < 2; ++y)// Search Strong Link Line wise
        ...
        {                    // Search Strong Link first Cell position Line wise
          if (!g[K[2] = w[a][Y]] || !(g[K[2]] & ~(g[w[K[2]][W[3][y]]] | g[w[K[2]][W[4][y]]]) & K[1]) ||
                             // Skip for Strong Link first Cell either position not unsolved; or ERI digit not in Clues; or ERI digit in mini-Line Cells Clues; or
            ...
            (!(g[w[K[2]][W[1][y] + 4]] & K[1]) << 4) | (!(g[w[K[2]][W[1][y] + 5]] & K[1]) << 5))] != 5 ||
                             // No ERI digit in not five Line Cell positions; or
            !(g[K[4] = w[a][W[1][y] + b[~K[3] & 63] - 1]] & K[1]))
            continue;        // No elimination
#if RJ > 2
          printf ("%d) Found Empty Rectangle Intersection Cell %d Strong Link %s Cells %d %d drop Clue %d from Cell %d\n",
            p, a, y ? "Column" : "Row", K[2], w[K[2]][W[1][y] + b[~K[3] & 63] - 1], b[K[1]], K[4]);
          if (!(g[w[K[4]][W[3][1 - y]]] & K[1]) + !(g[w[K[4]][W[4][1 - y]]] & K[1]) +
            !(g[w[K[4]][W[1][1 - y]]] & K[1]) + !(g[w[K[4]][W[1][1 - y] + 1]] & K[1]) +
            !(g[w[K[4]][W[1][1 - y] + 2]] & K[1]) + !(g[w[K[4]][W[1][1 - y] + 3]] & K[1]) +
            !(g[w[K[4]][W[1][1 - y] + 4]] & K[1]) + !(g[w[K[4]][W[1][1 - y] + 5]] & K[1]) == 5)
          {                  // Check no ERI digit in five second Line Cell positions
            printf ("%d) Found Empty Rectangle Intersection Cell %d Strong Link %s Cells %d %d drop Clue %d from Cell %d\n",
              p, a, y ? "Column" : "Row", K[4], w[K[2]][W[1][y] + b[~K[3] & 63] - 1], b[K[1]], K[2]);
            g[K[2]] &= ~K[1];// Drop ERI Strong Link digit from Line Cell position
          }
#endif
          g[K[4]] &= ~K[1];  // Drop ERI Strong Link digit from Line Cell position
          if (solve (p))
            return 1;
#if RJ > 2
          printf ("%d) Restore Empty Rectangle Intersection Cell %d Strong Link %s Cells %d %d add Clue %d to Cell %d\n",
            p, a, y ? "Column" : "Row", K[2], w[K[2]][W[1][y] + b[~K[3] & 63] - 1], b[K[1]], K[4]);
          if (!(g[w[K[4]][W[3][1 - y]]] & K[1]) + !(g[w[K[4]][W[4][1 - y]]] & K[1]) +
            !(g[w[K[4]][W[1][1 - y]]] & K[1]) + !(g[w[K[4]][W[1][1 - y] + 1]] & K[1]) +
            !(g[w[K[4]][W[1][1 - y] + 2]] & K[1]) + !(g[w[K[4]][W[1][1 - y] + 3]] & K[1]) +
