...
int q,                       // Number of unsolved Cell positions Grid wise
    ...
    s[60][81],               // s[0] = Sudoku Grid 81 Cell positions for clues and solved, s[1] - s[59] = Candidates Guess wise
    g[60],                   // g[0] - g[59] = Guess Cell positions
...
int solve (void)
{
  int a,
      ...
      Y,
      x,                     // Current unsolved Cell position for first time least Candidates checking
      y,                     // 0 = Naked Single, 1 = Hidden Single, 2 = Trial and Error
      z;                     // Current unsolved Cell Candidates

RESTART:
  for (y = 0, z = 512, a = 0; a < q; ++a)
  {                          // Search Naked/Hidden Single or Guess Candidate for each unsolved Cell position
    if (B[z] > B[s[p][r[a]]])// Check unsolved Cell position for least Candidates
      if (B[z = s[p][r[x = a]]] == 1)
    ...
    Y = s[p][r[a]] & ((s[p][r[a]] ^ (s[p][w[r[a]][0]] | s[p][w[r[a]][1]] |
      ...
      s[p][w[r[a]][19]])));  // Assign Hidden Single Candidates
...
/*
  ...
  for (y = 0; y < 54; ++y)   // Search Intersection Removal for 54 Box-Line 3 Cell positions
    ...
    {                        // Search Box-Line 3 Cell positions for each Intersection Removal Candidate
      ...
      {                      // Intersection Removal Candidate found in Line other Cell positions
        if ((s[p][j[y][9]] | s[p][j[y][10]] | s[p][j[y][11]] | s[p][j[y][12]] |
		  s[p][j[y][13]] | s[p][j[y][14]]) & a)
          continue;          // Intersection Removal Candidate found in Box other Cell positions
                             // Remove Intersection Removal Candidate from Line other Cell positions
        s[p][j[y][3]] &= ~a;
        s[p][j[y][4]] &= ~a;
        s[p][j[y][5]] &= ~a;
        s[p][j[y][6]] &= ~a;
      ...
      if (!((s[p][j[y][9]] | s[p][j[y][10]] | s[p][j[y][11]] | s[p][j[y][12]] |
        s[p][j[y][13]] | s[p][j[y][14]]) & a))
        continue;            // Intersection Removal Candidate not found in Box other Cell positions
      s[p][j[y][9]] &= ~a;   // Remove Intersection Removal Candidate from Box other Cell positions
      s[p][j[y][10]] &= ~a;
      s[p][j[y][11]] &= ~a;
      s[p][j[y][12]] &= ~a;
      s[p][j[y][13]] &= ~a;
  ...
  for (Y = 0; Y < 10; Y += 9)// Search Basic Fishes for Row and Column wise
    for (y = 0; y < 9; ++y)  // Search Basic Fishes for each Candidate
    {
      ...
      for (a = Y; a < Y + 9; ++a)
        k[a - Y] = (s[p][l[a][0]] & J[y] ? 1 : 0) | (s[p][l[a][1]] & J[y] ? 2 : 0) |
          (s[p][l[a][2]] & J[y] ? 4 : 0) | (s[p][l[a][3]] & J[y] ? 8 : 0) |
          (s[p][l[a][4]] & J[y] ? 16 : 0) | (s[p][l[a][5]] & J[y] ? 32 : 0) |
          (s[p][l[a][6]] & J[y] ? 64 : 0) | (s[p][l[a][7]] & J[y] ? 128 : 0) |
          (s[p][l[a][8]] & J[y] ? 256 : 0);
      for (a = 0; a < 36; ++a)
...
int check (int p, int q)
{
  ...
  for (; a < 20; ++a)
    if (s[0][w[p][a]] == q)  // Check duplicate Candidate in 20 affected clue Cell positions
      return 0;
  return q;
}
...
int invalid (void)
{
  ...
  for (; p < 81; ++p)
    if (!s[0][p])            // Check unsolved Cell position
    {
      ...
      if (!s[1][p])          // Check no Candidate assigned
        return 1;
