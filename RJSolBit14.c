...
int solve (int p)
{
  ...
  for (a = p; a < q; ++a)    // Search Naked Single Cell Clue or Guess minimum Cell Clues for each unsolved Cell position
    if (B[z] > B[g[r[a]]])   // Check least Cell Clues in current unsolved Cell position
      if (B[z = g[r[x = a]]] < 2)
        goto NHSCF;          // Naked Single Cell Clue found in current unsolved Cell position
  for (a = p; a < q; ++a)    // Search Hidden Single Cell Clue for each unsolved Cell position
  {
    ...
    if (!Y)                  // Check Hidden Single Cell Clue not found in unsolved Cell position
      continue;
  ...
  for (; y < 27; ++y)        // Search Naked/Hidden Tuples Cell Clues for each Unit
  {
    ...
    for (a = 0; a < 36; ++a) // Search Naked/Hidden pair Cell Clues for each Unit 36 pair Cell positions
    {
      ...
      if (B[K[0]] == 2)      // Check Naked pair Cell Clues found in Unit pair Cell positions
      {
        if (!(K[0] & K[1]))  // Check Naked pair Cell Clues not found in Unit other Cell positions
          continue;
        int k[7] = {g[l[y][h[a][2]]], g[l[y][h[a][3]]], g[l[y][h[a][4]]],
          g[l[y][h[a][5]]], g[l[y][h[a][6]]], g[l[y][h[a][7]]], g[l[y][h[a][8]]]};
                             // Backup and remove Naked pair Cell Clues from Unit other Cell positions
        g[l[y][h[a][2]]] &= ~K[0];
        g[l[y][h[a][3]]] &= ~K[0];
        g[l[y][h[a][4]]] &= ~K[0];
        g[l[y][h[a][5]]] &= ~K[0];
        ...
        g[l[y][h[a][8]]] &= ~K[0];
#if RJ > 2
        printf ("%d) Found Naked pair Cells %d %d Unit %d remove Clues %d from Cells %d %d %d %d %d %d %d\n",
      ...
      if (B[K[0] &= K[0] ^ K[1]] != 2)
        continue;            // Check Hidden pair Cell Clues not found in Unit pair Cell positions
      int k[2] = {g[l[y][h[a][0]]], g[l[y][h[a][1]]]};
                             // Backup and remove other than Hidden pair Cell Clues from Unit pair Cell positions
      g[l[y][h[a][0]]] &= K[0];
      g[l[y][h[a][1]]] &= K[0];
#if RJ > 2
    ...
    for (; a < 120; ++a)     // Search Naked/Hidden triplet Cell Clues for each Unit 84 triplet Cell positions
    {
      ...
      if (B[K[0]] == 3)      // Check Naked triplet Cell Clues found in Unit triplet Cell positions
      {
        if (!(K[0] & K[1]))  // Check Naked triplet Cell Clues not found in Unit other Cell positions
          continue;
        int k[6] = {g[l[y][h[a][3]]], g[l[y][h[a][4]]], g[l[y][h[a][5]]],
          g[l[y][h[a][6]]], g[l[y][h[a][7]]], g[l[y][h[a][8]]]};
                             // Backup and remove Naked triplet Cell Clues from Unit other Cell positions
        g[l[y][h[a][3]]] &= ~K[0];
        g[l[y][h[a][4]]] &= ~K[0];
        g[l[y][h[a][5]]] &= ~K[0];
        g[l[y][h[a][6]]] &= ~K[0];
        ...
        g[l[y][h[a][8]]] &= ~K[0];
#if RJ > 2
        printf ("%d) Found Naked triplet Cells %d %d %d Unit %d remove Clues %d from Cells %d %d %d %d %d %d\n",
      ...
      if (B[K[0] &= K[0] ^ K[1]] != 3)
        continue;            // Check Hidden triplet Cell Clues not found in Unit triplet Cell positions
      int k[3] = {g[l[y][h[a][0]]], g[l[y][h[a][1]]], g[l[y][h[a][2]]]};
                             // Backup and remove other than Hidden triplet Cell Clues from Unit triplet Cell positions
      g[l[y][h[a][0]]] &= K[0];
      g[l[y][h[a][1]]] &= K[0];
      g[l[y][h[a][2]]] &= K[0];
#if RJ > 2
    ...
    for (; a < 246; ++a)     // Search Naked/Hidden quad Cell Clues for each Unit 126 quad Cell positions
    {
      ...
      if (B[K[0]] == 4)      // Check Naked quad Cell Clues found in Unit quad Cell positions
      {
        if (!(K[0] & K[1]))  // Check Naked quad Cell Clues not found in Unit other Cell positions
          continue;
        int k[5] = {g[l[y][h[a][4]]], g[l[y][h[a][5]]], g[l[y][h[a][6]]],
          g[l[y][h[a][7]]], g[l[y][h[a][8]]]};
                             // Backup and remove Naked quad Cell Clues from Unit other Cell positions
        g[l[y][h[a][4]]] &= ~K[0];
        g[l[y][h[a][5]]] &= ~K[0];
        g[l[y][h[a][6]]] &= ~K[0];
        g[l[y][h[a][7]]] &= ~K[0];
        g[l[y][h[a][8]]] &= ~K[0];
#if RJ > 2
        printf ("%d) Found Naked quad Cells %d %d %d %d Unit %d remove Clues %d from Cells %d %d %d %d %d\n",
      ...
      if (B[K[0] &= K[0] ^ K[1]] != 4)
        continue;            // Check Hidden quad Cell Clues not found in Unit quad Cell positions
      int k[4] = {g[l[y][h[a][0]]], g[l[y][h[a][1]]], g[l[y][h[a][2]]], g[l[y][h[a][3]]]};
                             // Backup and remove other than Hidden quad Cell Clues from Unit quad Cell positions
      g[l[y][h[a][0]]] &= K[0];
      g[l[y][h[a][1]]] &= K[0];
      g[l[y][h[a][2]]] &= K[0];
      g[l[y][h[a][3]]] &= K[0];
  ...
  for (a = 0; a < 54; ++a)   // Search Intersection Removal for 54 Box-Line 3 Cell positions
    ...
    {                        // Search Box-Line 3 Cell positions for each Intersection Removal Clue
      ...
      {                      // Check Intersection Removal Clue found in Line other Cell positions
        if ((g[j[a][9]] | g[j[a][10]] | g[j[a][11]] | g[j[a][12]] | g[j[a][13]] | g[j[a][14]]) & y)
          continue;          // Check Intersection Removal Clue found in Box other Cell positions
        int k[6] = {g[j[a][3]], g[j[a][4]], g[j[a][5]], g[j[a][6]], g[j[a][7]], g[j[a][8]]};
                             // Backup and remove Intersection Removal Clue from Line other Cell positions
        g[j[a][3]] &= ~y;
        g[j[a][4]] &= ~y;
        g[j[a][5]] &= ~y;
        g[j[a][6]] &= ~y;
        ...
        g[j[a][8]] &= ~y;
#if RJ > 2
        printf ("%d) Found Pointing and Claiming Intersection Removal Cells %d %d %d remove Clue %d from Cells %d %d %d %d %d %d\n",
...
#endif
        return 0;
      }                      // Intersection Removal Clue not found in Line other Cell positions
      if (~(g[j[a][9]] | g[j[a][10]] | g[j[a][11]] | g[j[a][12]] | g[j[a][13]] | g[j[a][14]]) & y)
        continue;            // Check Intersection Removal Clue not found in Box other Cell positions
      int k[6] = {g[j[a][9]], g[j[a][10]], g[j[a][11]], g[j[a][12]], g[j[a][13]], g[j[a][14]]};
                             // Backup and remove Intersection Removal Clue from Box other Cell positions
      g[j[a][9]] &= ~y;
      g[j[a][10]] &= ~y;
      g[j[a][11]] &= ~y;
      g[j[a][12]] &= ~y;
      ...
      g[j[a][14]] &= ~y;
#if RJ > 2
      printf ("%d) Found Box-Line Reduction Intersection Removal Cells %d %d %d remove Clue %d from Cells %d %d %d %d %d %d\n",
  ...
  for (Y = 0; Y < 10; Y += 9)// Search Basic Fishes Line wise
    ...
    {                        // Search Basic Fishes for each digit
      ...
      {                      // Search X-Wing Clue for Line 36 pair Cell positions
        ...
        if ((B[A] != 2) | (A != k[h[a][1]]))
        {                    // Skip for X-Wing Clue not found in Lines pair Cell positions
          if (B[A] != 2)
            a += 7 - h[a][0];
          continue;
        }
        ...
        {                    // Search opposite Line other Cell positions for X-Wing Clue
          if (~(k[h[a][2]] | k[h[a][3]] | k[h[a][4]] | k[h[a][5]] | k[h[a][6]] |
            k[h[a][7]] | k[h[a][8]]) & K[Z])
            continue;        // Skip for X-Wing Clue not found in opposite Line other Cell positions
          X[Z] = b[K[Z]] - Y + 8;
          g[l[X[Z]][h[a][2]]] &= ~y;
          g[l[X[Z]][h[a][3]]] &= ~y;
          g[l[X[Z]][h[a][4]]] &= ~y;
          ...
          g[l[X[Z]][h[a][7]]] &= ~y;
          g[l[X[Z]][h[a][8]]] &= ~y;
        }                    // Remove X-Wing Clue from opposite Line other Cell positions
        if ((X[0] & X[1]) < 0)
          continue;          // Skip for X-Wing Clue not found in opposite Lines other Cell positions
#if RJ > 2
      ...
      for (; a < 120; ++a)   // Search Sword Fish Clue in Line 84 triplet Cell positions
      {
        ...
        {                    // Skip for either no unsolved Cell positions or Sord Fish Clue not found in Lines triplet Cell positions
        ...
        {                    // Search opposite Line other Cell positions for Sword Fish Clue
          if (~(k[h[a][3]] | k[h[a][4]] | k[h[a][5]] | k[h[a][6]] | k[h[a][7]] |
            k[h[a][8]]) & K[Z])
            continue;        // Skip for Sword Fish Clue not found in opposite Line other Cell positions
          X[Z] = b[K[Z]] - Y + 8;
          g[l[X[Z]][h[a][3]]] &= ~y;
          g[l[X[Z]][h[a][4]]] &= ~y;
          g[l[X[Z]][h[a][5]]] &= ~y;
          ...
          g[l[X[Z]][h[a][7]]] &= ~y;
          g[l[X[Z]][h[a][8]]] &= ~y;
        }                    // Remove Sword Fish Clue from opposite Line other Cell positions
        if ((X[0] & X[1] & X[2]) < 0)
          continue;          // Skip for Sword Fish Clue not found in opposite Lines other Cell positions
#if RJ > 2
      ...
      for (; a < 246; ++a)   // Search Jelly Fish Clue in Line 126 quad Cell positions
      {
        ...
        {                    // Skip for either no unsolved Cell positions or Jelly Fish Clue not found in Line quad Cell positions
        ...
        {                    // Search opposite Line other Cell positions for Jelly Fish Clue
          if (~(k[h[a][4]] | k[h[a][5]] | k[h[a][6]] | k[h[a][7]] | k[h[a][8]]) & K[Z])
            continue;        // Skip for Jelly Fish Clue not found in opposite Line other Cell positions
          X[Z] = b[K[Z]] - Y + 8;
          g[l[X[Z]][h[a][4]]] &= ~y;
          g[l[X[Z]][h[a][5]]] &= ~y;
          g[l[X[Z]][h[a][6]]] &= ~y;
          g[l[X[Z]][h[a][7]]] &= ~y;
          g[l[X[Z]][h[a][8]]] &= ~y;
        }                    // Remove Jelly Fish Clue from opposite Line other Cell positions
        if ((X[0] & X[1] & X[2] & X[3]) < 0)
          continue;          // Skip for Jelly Fish Clue not found in opposite Lines other Cell positions
#if RJ > 2
  ...
  for (a = p; a < q; ++a)    // Search XY-Wings Type 1 and XY-Wings Type 1 Transport unsolved Cell position wise
  {
    ...
    for (; K[1] < 6; ++K[1]) // Search first Wing Cell position Row wise
    {
      if ((B[g[K[2] = w[K[0]][K[1]]]] != 2) | (B[g[K[0]] & g[K[2]]] != 1))
        continue;            // Skip for first Wing Cell Clues not two digits; or Apex and first Wing Cells Clues not one digit
      for (K[3] = 14; K[3] < 20; ++K[3])
      {                      // Search second Wing Cell position Column wise
        if ((B[g[K[4] = w[K[0]][K[3]]]] != 2) |
                             // Skip for second Wing Cell Clues not two digits; or
          (B[K[5] = g[K[2]] & g[K[4]]] != 1) |
                             // Wing Cells common Clue not one digit; or
          (g[K[0]] & K[5]) | // Apex contain Wing Cells common Clue; or
          (B[g[K[0]] | g[K[2]] | g[K[4]]] != 3))
          continue;          // Apex and Wing Cells Clues not three digits
        if (g[K[6] = K[2] - K[0] + K[4]] & K[5])
        {                    // Check XY-Wings Type 1 elimination Cell Clues
          g[K[6]] &= ~K[5];  // Remove Wing Cells common Clue from XY-Wings Type 1 elimination Cell position
#if RJ > 2
          printf ("%d) Found XY-Wings Type 1 Cells %d %d %d Clues %d remove Clue %d from Cell %d\n",
          ...
          printf ("%d) Restore XY-Wings Type 1 Cells %d %d %d Clues %d add Clue %d to Cell %d\n",
            a, K[0], K[2], K[4], b[g[K[2]] | g[K[4]]], b[K[5]], K[6]);
#endif
          g[K[6]] |= K[5];   // Restore Wing Cells common Clue to XY-Wings Type 1 elimination Cell Clues
#if RJ > 3
          prn ();
#endif
          return 0;
        ...
        {                    // Check Wing cells common Clue found in Apex and first Wing common line but not box (01, 05, 09)
          if ((g[w[K[0]][y = 12]] & (g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            ~(g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]) ||
            (g[w[K[0]][++y]] & ~(g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            (g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]))
                             // Check Wing cells common Clue found in one line of box within chute neither Apex and first Wing common line nor box (05, 09)
            k[0] = w[K[0]][y];
...
if ((k[0] & k[1]) < 0)
{
        ...
        {                    // Check Wing cells common Clue found in Apex and second Wing common line but not box (02, 06, 10)
          if ((g[w[K[0]][y = 6]] & (g[w[Y][6]] | g[w[Y][8]] | g[w[Y][10]]) &
            ~(g[w[Y][7]] | g[w[Y][9]] | g[w[Y][11]]) & K[5]) ||
            (g[w[K[0]][++y]] & ~(g[w[Y][6]] | g[w[Y][8]] | g[w[Y][10]]) &
            (g[w[Y][7]] | g[w[Y][9]] | g[w[Y][11]]) & K[5]))
                             // Check Wing cells common Clue found in one line of box within chute neither Apex and first Wing common line nor box (06, 10)
            k[0] = w[K[0]][y];
...
if ((k[0] & k[1]) < 0)
{
        ...
        {                    // Check Wing cells common Clue found in second Wing and elimination common line but not box (03, 07, 11)
          if ((g[w[K[6]][y = 12]] & (g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            ~(g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]) ||
            (g[w[K[6]][++y]] & ~(g[w[Y][8]] | g[w[Y][9]] | g[w[Y][12]]) &
            (g[w[Y][10]] | g[w[Y][11]] | g[w[Y][13]]) & K[5]))
                             // Check Wing cells common Clue found in one line of box within chute neither Apex and first Wing common line nor box (07, 11)
            k[0] = w[K[6]][y];
...
if ((k[0] & k[1]) < 0)
{
        ...
        {                    // Check Wing cells common Clue found in first Wing and elimination common line but not box (04, 08, 12)
          if ((g[w[K[6]][y = 6]] & (g[w[Y][6]] | g[w[Y][8]] | g[w[Y][10]]) &
            ~(g[w[Y][7]] | g[w[Y][9]] | g[w[Y][11]]) & K[5]) ||
            (g[w[K[6]][++y]] & ~(g[w[Y][6]] | g[w[Y][8]] | g[w[Y][10]]) &
            (g[w[Y][7]] | g[w[Y][9]] | g[w[Y][11]]) & K[5]))
                             // Check Wing Cells common Clue found in one line of box within chute neither Apex and Wing common line nor box (08, 12)
            k[0] = w[K[6]][y];
        ...
        if ((k[0] & k[1]) > -1)
        {
          ...
          if (k[1] > -1)
            g[k[1]] &= ~K[5];
#if RJ > 2
          printf ("%d) Found XY-Wings Type 1 Transport %s Cells %d %d %d %d Clues %d remove Clue %d from Cell",
            ...
            if (~(g[Y = w[w[K[0]][12]][K[7]]] | g[Y + 1] | g[Y + 2]) & K[5])
            {
              ...
              {              // (17)
                if ((~(g[w[K[0]][8]] | g[w[K[0]][9]]) & K[5]) &&
                  ...
                  g[Y = w[K[0]][K[7]]] | g[Y + 1] | g[Y + 2]) & g[K[0]] & g[K[2]]) ||
                  ((g[w[K[0]][12]] | g[w[K[0]][13]] | g[w[K[4]][12]] | g[w[K[4]][13]] |
                  g[Y = w[K[0]][K[8]]] | g[Y + 9] | g[Y + 18]) & g[K[0]] & g[K[4]])))
                {            // (17)
                  k[0] = y;
                  k[1] = y + 9;
                  k[2] = y + 18;
...
if ((k[2] & k[3]) < 0)
{
        ...
        if ((k[2] & k[3]) > -1)
        {
          ...
          if (k[2] > -1)
          {
            ...
            if (k[4] > -1)
              printf (" %d %d %d %d %d %d\nremove Clue %d from Cells %d %d %d %d %d %d %d remove Clue %d from Cells %d %d %d %d %d %d %d",
...
/**/
        if ((((g[w[K[0]][6]] | g[w[K[0]][7]]) & (g[w[K[0]][12]] | g[w[K[0]][13]]) &
          ...
          g[Y = w[K[0]][K[8]]] | g[Y + 9] | g[Y + 18]) & K[5]) ||
          ...
          (((g[y = w[K[4]][K[7]]] | g[y + 1] | g[y + 2] | g[w[K[4]][6]] |
          g[w[K[4]][7]] | g[Y = w[K[4]][K[7]]] | g[Y + 9] | g[Y + 18] |
          g[w[K[2]][12]] | g[w[K[2]][13]]) & K[5]) ||
          ((g[w[K[0]][6]] | g[w[K[0]][7]] | g[w[K[2]][6]] | g[w[K[2]][7]] |
...
/**/
  ...
  for (y = 0; y < 2; ++y)    // Search XY-Wings Type 2, XYZ-Wings and XYZ-Wings Hybrid Type 1 and Type 2 Chute wise
    ...
    {
      if ((B[g[r[a]]] > 3) | // Skip for unsolved Cell Clues more than three digits; or
        ...
        continue;            // Either Apex Box or Line contain no unsolved Cell position
      int K[5] = {r[a], W[0][y]};
                             // Assign Apex Cell position
      for (; K[1] < W[1][y]; ++K[1])
      {                      // Search first Wing Cell position Box wise
        if ((B[g[w[K[0]][K[1]]]] != 2) | (B[g[K[0]] | g[w[K[0]][K[1]]]] != 3))
          continue;          // Skip for either Box Wing Cell Clues not two digits or Apex and Box Wing Cell Clues not three digits
        ...
        {                    // Search second Wing Cell position Line wise
          if ((B[g[w[K[0]][K[2]]]] != 2) | (B[g[K[0]] | g[w[K[0]][K[1]]] | g[w[K[0]][K[2]]]] != 3) |
                             // Skip for either Line Wing Cell Clues not two digits or Cells Clues not three digits; or
            (B[K[4] = g[w[K[0]][K[1]]] & g[w[K[0]][K[2]]]] != 1))
            continue;        // Wing Cells common Clue not one digit
          K[3] = W[K[2] < W[6][y] ? 2 : 6][y];
                             // Assign Wing Cells common first elimination Cell position
          if (!(g[K[0]] & K[4]))
          {                  // Check Apex not contain Wing Cells common Clue for XY-Wings Type 2
            if ((g[w[K[0]][W[4][y]]] | g[w[K[0]][W[5][y]]] |
              g[w[w[K[0]][K[1]]][K[3]]] | g[w[w[K[0]][K[1]]][K[3] + 1]] |
              g[w[w[K[0]][K[1]]][K[3] + 2]]) & K[4])
            {                // Check XY-Wings Type 2 eliminations Cell Clues
              int k[5] = {g[w[K[0]][W[4][y]]], g[w[K[0]][W[5][y]]], g[w[w[K[0]][K[1]]][K[3]]],
                g[w[w[K[0]][K[1]]][K[3] + 1]], g[w[w[K[0]][K[1]]][K[3] + 2]]};
                             // Backup and remove Wing Cells common Clue from XY-Wings Type 2 elimination Cell positions
              g[w[K[0]][W[4][y]]] &= ~K[4];
              g[w[K[0]][W[5][y]]] &= ~K[4];
              ...
              g[w[w[K[0]][K[1]]][K[3] + 2]] &= ~K[4];
#if RJ > 2
              printf ("%d) Found XY-Wings Type 2 Cells %d %d %d Clues %d remove Clue %d from Cells %d %d %d %d %d\n",
              ...
              if (solve (p))
                return 1;
#if RJ > 2
              printf ("%d) Restore XY-Wings Type 2 Cells %d %d %d Clues %d add Clue %d to Cells %d %d %d %d %d\n",
                a, K[0], w[K[0]][K[1]], w[K[0]][K[2]], b[g[w[K[0]][K[1]]] | g[w[K[0]][K[2]]]],
                b[K[4]], w[K[0]][W[4][y]], w[K[0]][W[5][y]],
                w[w[K[0]][K[1]]][K[3]], w[w[K[0]][K[1]]][K[3] + 1], w[w[K[0]][K[1]]][K[3] + 2]);
#endif
              ...
              prn ();
#endif
              return 0;      // Restore XY-Wings Type 2 elimination Cells Clues
            }
// XY-Wings Type 2 Transport
          }
          if (B[g[K[0]]] != 3)
            continue;        // Skip for Apex Cell Clues not three digits
          if ((g[w[K[0]][W[4][y]]] | g[w[K[0]][W[5][y]]]) & K[4])
          {                  // Check XYZ-Wings elimination Cells Clue
            int k[2] = {g[w[K[0]][W[4][y]]], g[w[K[0]][W[5][y]]]};
                             // Backup and remove Wing Cells common Clue from XYZ-Wings elimination Cell positions
            g[w[K[0]][W[4][y]]] &= ~K[4];
            g[w[K[0]][W[5][y]]] &= ~K[4];
#if RJ > 2
            printf ("%d) Found XYZ-Wings Cells %d %d %d Clues %d remove Clue %d from Cells %d %d\n",
            ...
            if (solve (p))
              return 1;
#if RJ > 2
            printf ("%d) Restore XYZ-Wings Cells %d %d %d Clues %d add Clue %d to Cells %d %d\n",
              a, K[0], w[K[0]][K[1]], w[K[0]][K[2]], b[g[K[0]]],
              b[K[4]], w[K[0]][W[4][y]], w[K[0]][W[5][y]]);
#endif
            g[w[K[0]][W[4][y]]] = k[0];
            ...
            prn ();
#endif
            return 0;        // Restore XYZ-Wings elimination Cells Clues
          }
          for (Y = 0; Y < 2; ++Y)
          {                  // Search XYZ-Wings Hybrid Type wise
            ...
            k[1] = g[w[K[0]][X[1]]];
            k[2] = g[w[K[0]][X[2]]];
                             // Backup and remove Wing Cells common Clue from XYZ-Wings Hybrid Type 1 and Type 2 elimination Cell positions
            g[w[K[0]][X[0]]] &= ~K[4];
            g[w[K[0]][X[1]]] &= ~K[4];
            g[w[K[0]][X[2]]] &= ~K[4];
#if RJ > 2
            printf ("%d) Found XYZ-Wings Hybrid Type %d Cells %d %d %d %d Clues %d remove Clue %d from Cells %d %d %d\n",
            ...
            prn ();
#endif
            return 0;        // Restore XYZ-Wings Hybrid Type 1 and Type 2 elimination Cells Clues
          }
// XYZ-Wings Transport
        }
      }
  ...
  {
    if ((B[g[r[a]]] != 3) |  // Skip for unsolved Cell Clues not three digits; or
      !(g[w[r[a]][0]] | g[w[r[a]][1]] | g[w[r[a]][2]] |
      g[w[r[a]][3]] | g[w[r[a]][4]] | g[w[r[a]][5]]) |
      !(g[w[r[a]][14]] | g[w[r[a]][15]] | g[w[r[a]][16]] |
      g[w[r[a]][17]] | g[w[r[a]][18]] | g[w[r[a]][19]]))
    ...
    for (; K[1] < 6; ++K[1]) // Search first Wing Cell position Row wise
    {
      if (!g[w[K[0]][K[1]]] | (B[g[w[K[0]][K[1]]]] > 3))
        continue;            // Skip for first Wing Cell either position not unsolved or Clues more than three digits
      for (K[2] = 14; K[2] < 20; ++K[2])
      {                      // Search second Wing Cell position Column wise
        if (!g[w[K[0]][K[2]]] | (B[g[w[K[0]][K[2]]]] > 3))
          continue;          // Skip for second Wing Cell either position not unsolved or Clues more than three digits
        for (y = 0; y < 2; ++y)
          ...
          {                  // Search third Wing Cell position
            if (!g[w[K[0]][K[3]]] | (B[g[w[K[0]][K[3]]]] > 3) |
                             // Skip for third Wing Cell either position not unsolved or Clues more than three digits; or
              (B[K[4] = g[w[K[0]][K[2 - y]]] & (g[w[K[0]][K[1 + y]]] | g[w[K[0]][K[3]]])] != 1) |
                             // Wing Cells common Clue not one digit; or
              (B[g[K[0]] | K[4]] != 4) |
                             // Apex and Wing Cells Clues not four digits; or
              (B[g[w[K[0]][K[2 - y]]]] != 2) |
                             // Line (or Box) Wing Cell Clues not two digits; or
              (B[g[K[0]] | g[w[K[0]][K[2 - y]]]] != 4) |
                             // Apex and Line (or Box) Wing Cells Clues not four digits; or
              (B[g[w[K[0]][K[1 + y]]] | g[w[K[0]][K[3]]]] != 3) |
                             // Box (or Line) Wing Cell Clues and third Wing Cell Clues not three digits; or
              (B[g[K[0]] | g[w[K[0]][K[1 + y]]] | g[w[K[0]][K[3]]]] != 4) |
                             // Apex Cell Clues, Box (or Line) Wing Cell Clues and third Wing Cell Clues not four digits; or
              (B[g[K[0]] | g[w[K[0]][K[1 + y]]]] == B[g[K[0]] | g[w[K[0]][K[3]]]]))
              continue;      // Apex Cell Clues and first (or second) Wing Cell Clues equals to Apex Cell Clues and third Wing Cell Clues
            if (g[K[5] = w[K[0]][K[g[w[K[0]][K[1]]] & K[4] ? 1 : 3]] - K[0] +
              w[K[0]][K[g[w[K[0]][K[2]]] & K[4] ? 2 : 3]]] & K[4])
            {                // Search WXYZ-Wings Type 1 elimination Cell Clues
                             // Remove Wing Cells common Clue from WXYZ-Wings Type 1 elimination Cell position
              g[K[5]] &= ~K[4];
#if RJ > 2
              printf ("%d) Found WXYZ-Wings Type 1 Cells %d %d %d %d Clues %d remove Clue %d from Cell %d\n",
              ...
              prn ();
#endif
              return 0;      // Restore Wing Cell common Clue to WXYZ-Wings Type 1 elimination Cell Clues
            }
  ...
  for (y = 0; y < 2; ++y)    // Search WXYZ-Wings Type 2a, Type 2b and Type 3 Chute wise
    for (a = p; a < q; ++a)  // Search unsolved Cell position wise
    {
      if ((B[g[r[a]]] < 3) | (B[g[r[a]]] > 4) |
                             // Skip for unsolved Cell Clues either less than three digits or more than four digits; or
      !(g[w[r[a]][W[0][y]]] | g[w[r[a]][W[0][y] + 1]] | g[w[r[a]][W[0][y] + 2]] |
      g[w[r[a]][W[0][y] + 3]] | g[w[r[a]][W[0][y] + 4]] | g[w[r[a]][W[0][y] + 5]]) |
      !(g[w[r[a]][W[2][y]]] | g[w[r[a]][W[2][y] + 1]] | g[w[r[a]][W[2][y] + 2]] |
      g[w[r[a]][W[2][y] + 3]] | g[w[r[a]][W[2][y] + 4]] | g[w[r[a]][W[2][y] + 5]]))
            ...
            {                // Search third Wing Cell position
              if (!g[w[K[0]][K[3]]] | (B[g[w[K[0]][K[3]]]] > 3) |
                ...
                (B[K[4] = g[w[K[0]][K[2 - Y]]] & (g[w[K[0]][K[1 + Y]]] | g[w[K[0]][K[3]]])] != 1) |
                             // Wing Cells common Clue not one digit; or
                (B[g[K[0]] | K[4]] != 4) |
                             // Apex Cell Clues and Wing Cells common Clue not four digits; or
                (B[g[w[K[0]][K[2 - Y]]]] != 2) |
                             // Line (or Box) Wing Cell Clues not two digits; or
                (B[g[K[0]] | g[w[K[0]][K[2 - Y]]]] != 4) |
                             // Apex and Line (or Box) Wing Cells Clues not four digits; or
                (B[g[w[K[0]][K[1 + Y]]] | g[w[K[0]][K[3]]]] != 3) |
                             // Box (or Line) and third Wing Cells Clues not three digits; or
                (B[g[K[0]] | g[w[K[0]][K[1 + Y]]] | g[w[K[0]][K[3]]]] != 4))
                continue;    // Apex, Box (or Line) Wing and third Wing Cells Clues not four digits
              int X[2] = {-1},
                  k[5] = {g[w[K[0]][W[4][y]]], g[w[K[0]][W[5][y]]]};
                             // Backup WXYZ-Wings Type 2b and Type 3 elimination Cell positions
              if ((B[g[K[0]]] == 3) &
                             // Check Apex Cell Clues three digits; and
                ((w[K[0]][K[3]] == w[w[K[0]][K[1 + Y]]][W[4][y]]) |
                (w[K[0]][K[3]] == w[w[K[0]][K[1 + Y]]][W[5][y]]) |
                             // Either third Wing Cell position within first (or second) Wing Cell position Box and Line; or
                (B[g[K[0]] | g[w[K[0]][K[1 + Y]]]] != B[g[K[0]] | g[w[K[0]][K[3]]]])))
              ...
              {              // Check WXYZ-Wings Type 2a, Type 2b and Type 3 eliminations Cell Clues
                             // Remove Wing Cells common Clue from WXYZ-Wings Type 2b and Type 3 elimination Cell positions
                g[w[K[0]][W[4][y]]] &= ~K[4];
                g[w[K[0]][W[5][y]]] &= ~K[4];
                if (X[0] > -1)
                {            // Remove Wing Cells common Clue from WXYZ-Wings Type 2a more elimination Cell positions
                  g[w[X[0]][X[1]]] &= ~K[4];
                  g[w[X[0]][X[1] + 1]] &= ~K[4];
                  g[w[X[0]][X[1] + 2]] &= ~K[4];
                }
#if RJ > 2
                if (X[0] > -1)
                  printf ("%d) Found WXYZ-Wings Type 2a Cells %d %d %d %d Clues %d remove Clue %d from Cells %d %d %d %d %d\n",
                    a, K[0], w[K[0]][K[1]], w[K[0]][K[2]], w[K[0]][K[3]], b[g[K[0]] | g[w[K[0]][K[2 - Y]]]],
                    b[K[4]], w[K[0]][W[4][y]], w[K[0]][W[5][y]], w[X[0]][X[1]], w[X[0]][X[1] + 1], w[X[0]][X[1] + 2]);
                else
                  printf ("%d) Found WXYZ-Wings Type %s Cells %d %d %d %d Clues %d remove Clue %d from Cells %d %d\n",
                    a, B[K[0]] == 3 ? "2b" : "3",
                ...
                prn ();
#endif
                return 0;    // Restore WXYZ-Wings Type 2a, Type 2b and Type 3 elimination Cells Clues
              }
...
NHSCF:
  ...
  {                          // Backup and clear current Cell Clues and assign current Cell position to each current Cell Clue
    ...
    for (++n[y], a = 0; a < 20; ++a)
      ...
      if (g[w[r[p]][a]] & s[r[p]])
      {
        k |= 1 << a;         // Backup and remove current Cell Clue from 20 peer Cell positions
        g[w[r[p]][a]] &= ~s[r[p]];
      }
#if RJ > 2
    ...
    prn ();
    g[r[p]] = 0;             // Remove current Cell Clues from current Cell position
#endif
  }
  if (y == 2)
    --n[3];
